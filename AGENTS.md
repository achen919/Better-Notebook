...
这是 `Better-Notebook`，一个基于艾宾浩斯遗忘曲线算法管理学习笔记和错题的 Electron 应用，使用 React、TypeScript 和 Vite 构建。
...
使用 sql.js（编译为 WebAssembly 的 SQLite）。数据库文件存储位置受 `electron-builder.json` 中的应用标识和产品名影响；如果你同步完成 `Better-Notebook` 重命名，请以打包配置中的实际值为准。
...
仓库中的 agent 开发流程统一放在 `.harness/` 目录：

- `.harness/templates/task.md`：任务输入模板
- `.harness/checklists/feature.md`：通用功能开发检查项
- `.harness/checklists/ipc-db.md`：涉及 Electron IPC / 数据库时必须追加检查
- `.harness/evals/smoke.yaml`：最小 smoke 验收场景

开始开发前，优先根据任务内容选择对应 checklist；提交前执行 `yarn harness`。
